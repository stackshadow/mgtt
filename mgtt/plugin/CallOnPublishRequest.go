...
func CallOnPublishRequest(clientID string, username string, topic string, payload string) (accepted bool) {
	...
	for pluginName, plugin := range pluginList {
		if plugin.OnPublishRequest != nil {
			...
			accepted = accepted && plugin.OnPublishRequest(clientID, username, topic, payload)
		}
	}
