...
COPY healthcheck/main.go healthcheck/go.mod healthcheck/go.sum /go/src/
WORKDIR /go/src
...
RUN CGO_ENABLED=0 GOOS=linux go build -a -ldflags '-extldflags "-static"' -o mgtt . && \
    ./mgtt -h
...
# infos
LABEL maintainer "stackshadow@evilbrain.de" \
      githash "${GITHASH}"
...
USER nobody
CMD ["/mgtt", "--debug", "--terminal", "--db-filename=/data/messages.db", "--cert-file=/data/cert.pem", "--key-file=/data/key.pem", "--config-path=/data/" ]
